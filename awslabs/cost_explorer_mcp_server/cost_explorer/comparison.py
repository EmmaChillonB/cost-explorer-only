...
"""Cost comparison handlers for the Cost Explorer package."""
...
def _validate_comparison_inputs(
    ...
    """Validate all comparison inputs and return validated parameters.

    Args:
        baseline_date_range: The reference period for comparison
        comparison_date_range: The comparison period
    ...
    is_valid, error = validate_comparison_date_range(baseline_start, baseline_end)
    if not is_valid:
        return False, f'Baseline period error: {error}', {}

    # Validate comparison period
    is_valid, error = validate_comparison_date_range(comparison_start, comparison_end)
    if not is_valid:
        return False, f'Comparison period error: {error}', {}
...
async def get_cost_comparison_drivers(
    ...
    Returns:
        Dictionary containing driver analysis with specific usage types, usage quantity changes,
        driver types (savings plans, discounts, usage changes, support fees), and contextual information
    """
    # Initialize variables for error handling
    baseline_start = baseline_date_range.start_date
    baseline_end = baseline_date_range.end_date
