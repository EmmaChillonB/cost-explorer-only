...
import pytest
from cost_optimizer.cost_explorer.comparison import (
    get_cost_and_usage_comparisons,
    get_cost_comparison_drivers,
)
from cost_optimizer.cost_explorer.models import DateRange
...
def mock_ce_client():
    ...
    with patch(
        'cost_optimizer.cost_explorer.comparison.get_cost_explorer_client'
    ) as mock:
        client = MagicMock()
...
class TestCostComparisons:
    ...
    async def test_get_cost_comparisons_with_tag_selector(
        ...
        with patch(
            'cost_optimizer.cost_explorer.comparison.validate_expression',
            return_value={},
        ):
            with patch(
                'cost_optimizer.cost_explorer.comparison.validate_group_by',
                return_value={'Type': 'TAG', 'Key': 'Environment'},
            ):
                with patch(
                    'cost_optimizer.cost_explorer.comparison.validate_comparison_date_range',
                    return_value=(True, ''),
                ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        'cost_optimizer.cost_explorer.comparison.extract_group_key_from_complex_selector',
                        return_value='Production',
                    ):
                        with patch(
                            'cost_optimizer.cost_explorer.comparison.extract_usage_context_from_selector',
                            return_value={'tag': 'Environment:Production'},
                        ):
                            with patch(
                                'cost_optimizer.cost_explorer.comparison.create_detailed_group_key',
