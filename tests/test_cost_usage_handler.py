...
import pytest
from cost_optimizer.cost_explorer.usage import get_cost_and_usage
...
def mock_ce_client():
    ...
    with patch(
        'cost_optimizer.cost_explorer.usage.get_cost_explorer_client'
    ) as mock:
        client = MagicMock()
...
class TestCostAndUsage:
    ...
    async def test_get_cost_and_usage_missing_amount(self, mock_ce_client, valid_date_range):
        ...
        with patch(
            'cost_optimizer.cost_explorer.usage.validate_expression',
            return_value={},
        ):
            with patch(
                'cost_optimizer.cost_explorer.usage.validate_group_by',
                return_value={},
            ):
    ...
    @pytest.mark.asyncio
    @patch('cost_optimizer.cost_explorer.usage.get_cost_explorer_client')
